...
fn expensive_sum(v: Vec<i32>) -> i32 {
  pause_ms(500);
  println!("Exercise H: Child thread: just about finished");
...
pub fn run() {
  ...
  for letter in vec!["a", "b", "c", "d", "e", "f"] {
    println!("Exercise H: Main thread: Letter {}", letter);
    pause_ms(200);
  }
  ...
  println!("Exercise H: The child thread's expensive sum is {}", sum);
  ...
  let handle_a = thread::spawn(move || {
    pause_ms(0);
    tx2.send("Exercise H: Thread A: 1").unwrap();
    pause_ms(200);
    tx2.send("Exercise H: Thread A: 2").unwrap();
  });
  ...
  let handle_b = thread::spawn(move || {
    pause_ms(0);
    tx.send("Exercise H: Thread B: 1").unwrap();
    pause_ms(200);
    tx.send("Exercise H: Thread B: 2").unwrap();
  });
  ...
  for msg in rx {
    println!("Exercise H: Main thread: Received {}", msg);
  }
  ...
  // the child threads.
  let (sendor, receiver) = channel::unbounded();
  let receivor2 = receiver.clone();

  let handle_c = thread::spawn(move || {
    for msg in receivor2 {
      println!("Exercise H: Thread C received: {}", msg);
    }
  });

  let handle_d = thread::spawn(move || {
    for msg in receiver {
      println!("Exercise H: Thread D received: {}", msg);
    }
  });
  ...
  println!("Exercise H: Main thread exited.")
}
