...
fn bunny_nibbles<T: Bite>(arg: &mut T) {
  for x in 1..=4 {
    arg.bite();
    println!("Exercise F: Bunny nibbles {} times", x);
  }
}
...
pub fn run() {
  ...
  carrot.bite();
  println!("Exercise F: I take a bite: {:?}", carrot);
  ...
  grapes.bite();
  println!("Exercise F: I ate a grape: {:?}", grapes);
  ...
  println!("Exercise F: Bunny nibbled for awhile: {:?}", carrot);
}
